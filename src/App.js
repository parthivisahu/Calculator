...
import moonIcon from "./images/moon_night.jpg";
import sunIcon from "./images/sun_day.png";
...
function App() {
  ...
  const handleKeyPress = (keyCode, key) => {
    ...
    if (numbers.includes(key)) {
      ...
      if (tempHistory.length > 3) tempHistory = tempHistory.splice(1);
      ...
      <div className="app_calculator">
        <div className="app_calculator_navbar">
          ...
          onClick={()=>setIsLightMode(!isLightMode)}>
            <div className={`app_calculator_navbar_toggle_circle ${ isLightMode ? "app_calculator_navbar_toggle_circle_active" : ""}`} />
            </div>
            <img src={isLightMode ? sunIcon : moonIcon } alt="mode" />
        </div>
